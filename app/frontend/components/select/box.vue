...
<template>
  <div ref="box" class="relative">
    ...
    >
      <ul
        v-if="optionsVisible"
        class="absolute mt-1 max-h-60 overflow-hidden rounded-md bg-base-100 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm d-join d-join-vertical"
      >
        <slot
          name="options"
          :hide-options="() => toggleOptionsVisible(false)"
