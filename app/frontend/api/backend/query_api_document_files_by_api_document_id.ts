...
export function queryApiDocumentFilesByApiDocumentId<STATUS extends keyof ResponseMap>(arg?: QueryArg & ParamArg & HeaderArg): Keq<ResponseMap[STATUS]> {
  const req = request.get<ResponseMap[STATUS]>
  ("/api/api-document/:apiDocumentId/api-document-file")
    .option('module', {
      name: "backend",
